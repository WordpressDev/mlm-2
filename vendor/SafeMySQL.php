...
class SafeMySQL
{
    ...
    {
        $settings = Functions::getConfig('db');
	...
	public function prepareQuery($args)
	{
		...
		$array = preg_split('~(\?[nsiuapth])~u',$raw,null,PREG_SPLIT_DELIM_CAPTURE);
		$anum  = count($args);
		...
		foreach ($array as $i => $part)
		{
			...
			switch ($part)
			{
				...
				case '?s':
					...
					break;
                case '?t':
                    $part = $this->escapeTaggedString($value);
                    break;
                case '?h':
                    $part = $this->escapeAnchorString($value);
                    break;
				case '?i':
	...
	private function escapeString($value)
	{
		...
		$res = htmlspecialchars("'".mysqli_real_escape_string($this->conn,$value)."'");

        return $res;
	}

    private function escapeTaggedString($value)
    {
        if ($value === NULL)
        {
            return 'NULL';
        }
        //$value = Functions::filterTags($value);
        $res = "'".mysqli_real_escape_string($this->conn,$value)."'";

        return $res;
    }

    private function escapeAnchorString($value)
    {
        if ($value === NULL)
        {
            return 'NULL';
